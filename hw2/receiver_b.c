...
int buf_arr_cnt;
char *buf_arr;
struct sockaddr_in clientAddress, agentAddress[AGENT];
FILE *fp;
void initBufStat(){
	buf_arr_cnt=0;
...
int main(int argc, char *argv[]){ //clientPort, agentPort*3, file
	if(argc!=6)
		die("argument");
	fp=fopen(argv[5],"wb+");
	...
	for(int i=0;i<AGENT;i++){
		bzero(&agentAddress[i], sizeof(agentAddress[i]));
		agentAddress[i].sin_family=AF_INET;
		agentAddress[i].sin_port = htons(atoi(argv[2+i]));
		agentAddress[i].sin_addr.s_addr=inet_addr("127.0.0.1");
	}
	agent_sz=sizeof(agentAddress[0]);
	bufSize=32;
	base=0;
	...
	while(1){	
		...
		if(strcmp(data.type,"fin")==0){  //is fin
			...
			if (sendto(udpfd, buf, sizeof(ack), 0, (struct sockaddr*)&agentAddress[0], agent_sz) == -1)
				die("sendto");
			if (sendto(udpfd, buf, sizeof(ack), 0, (struct sockaddr*)&agentAddress[1], agent_sz) == -1)
				die("sendto");
			if (sendto(udpfd, buf, sizeof(ack), 0, (struct sockaddr*)&agentAddress[2], agent_sz) == -1)
				die("sendto");
			printf("send    %-4s\n",ack.type);
			fflush(stdout);
			...
			strcpy(ack.type,"ack");
			memcpy(buf,&ack,sizeof(ack));
			if (sendto(udpfd, buf, sizeof(ack), 0, (struct sockaddr*)&agentAddress[ack.id%AGENT], agent_sz) == -1)
				die("sendto");
			printf("send    %-4s    #%d\n",ack.type,ack.id);
			fflush(stdout);
		}
