...
int file_sz;
char buf[BUFLEN+1];
struct sockaddr_in serverAddress, agentAddress[AGENT];
FILE *fp;
size_t res;
void initAckStat(){
	free(ack_mask);
...
void sendData(int type){
	...
	for(int i=0;i<winSize;i++){
		...
		if (sendto(udpfd, buf, sizeof(data), 0, (struct sockaddr*)&agentAddress[data.id%AGENT], agent_sz) == -1)
			die("sendto");
...
void sendFin(int type){
	...
	memcpy(buf,&data,sizeof(data));
	data.payload_size=0;
	if (sendto(udpfd, buf, sizeof(data), 0, (struct sockaddr*)&agentAddress[0], agent_sz) == -1)
		die("sendto");
...
int main(int argc, char* argv[]){ //serverPort, agentPort*3, file
	if(argc!=6)
		die("argument");
	fp=fopen(argv[5],"rb");
	...
	while(1){
		...
		if(recv_len == -1){
			...
			if(ack_cnt==winSize){ //can increase window size
				canSend=1;
				base+=winSize;
				alarm(0); //stop timer
				//window size of next round
